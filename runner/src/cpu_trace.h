...
/* Write-site PC for WRAM-watch events. Declared UNCONDITIONALLY, unlike
 * everything else below this comment: interp_bridge.c reads and writes these
 * on every interpreted step regardless of SNESRECOMP_TRACE, so the symbols
 * must always exist (defined unconditionally in cpu_trace.c to match).
 *
